...
namespace Server.Items
{
    ...
    {
        private static readonly TimeSpan delay = TimeSpan.FromMinutes( 15 );
        private static readonly TimeSpan totalDelay = TimeSpan.FromMinutes( 15 );
        ...
        {
			int hs_status=(int)HomeStone.GetRemainingTimeToUseForMobile( from ).TotalMinutes;
			string hs_time="minutes";
			if (hs_status <1)
			{
				hs_status=(int)HomeStone.GetRemainingTimeToUseForMobile( from ).TotalSeconds; 
				hs_time="seconds";
			}
            if ( from != owner )
            {
            ...
            {
                from.SendMessage( "Your homestone will be ready again in {0} {1}!", hs_status, hs_time );
                return false;
            }
